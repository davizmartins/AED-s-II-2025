...
class hashRehash {
    ...
    //Funçao para obeter o valor ASCII da string para jogar na função de posição
    public int valorASC(String s) {
        int somaTotal = 0;
        if (s != null) {
            // Itera sobre a string
            for (int i = 0; i < s.length(); i++) {
                // Converte o caractere para int e adiciona à soma total
                somaTotal += (int) s.charAt(i);
            }
...
public class Q02 {
    ...
    public static void printarSaida(String nm, boolean resp, int p){
        ...
        System.out.println(nm + ":  (Posicao: " + p + ") " + str1);
    }
    ...
    public static Game converterParaGame(String linha) {
        ...
        float uScore = (partes[7].isEmpty() || partes[7].equalsIgnoreCase("tbd")) ? -1.0f
                : Float.parseFloat(partes[7]);
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Faz a busca dos jogos pelo id e salva eles dentro da Hash
        hashRehash hashGames = new hashRehash();
        ...
        while (entrada.hasNextLine()) {
            ...
            boolean resp = hashGames.pesquisar(nmBusca);
            
            int pos = hashGames.hs(nmBusca);
            
            printarSaida(nmBusca, resp, pos);
        }
