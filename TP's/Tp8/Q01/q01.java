...
class hashReserva {
    ...
    public boolean pesquisar(Game g) {
        boolean resp = false;
        int pos = hs(g.getName());
        if (tabela[pos] == g) {
           resp = true;
    ...
    public int obterPosicao(Game g){
        int resp=-1;
        int pos = hs(g.getName());
        if (tabela[pos] == g) {
           resp = pos;
...
public class q01 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        hashReserva hashGames = new hashReserva();
        ...
        while (entrada.hasNextLine()) {
            ...
            Game aux;
            for (int k = 0; k < games.length; k++) {
                if (games[k] != null && games[k].getName() == nmBusca) {
                    aux=games[k];
                    break;
                }
            }

            boolean resp= hashGames.pesquisar(aux);
            int pos; 
            if(resp==true){
                pos=hashGames.obterPosicao(aux);
            }
